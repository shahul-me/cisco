...
class RouterController extends Controller
{
    ...
    {
        if ($request->isMethod('get')) {
            $input = $request->only('token', 'range1', 'range2','login_id');
            $validator = \Validator::make($input, [
                'type' => 'required',
                'login_id' => 'required','range1' => 'required','range2' => 'required', 'token' => 'required']);
            if ($validator->passes()) {

                $result = User::where('login_id', trim($input['login_id']))->where('token', trim($input['token']))->first();
                if ($result) {
                    $result_data = Router::whereIn('loopback', [$input['range1'],$input['range2']])
                    ->get();
                    if (($result_data)) {
                        $response = ['code' => 200,  'data' => $result_data];
                    } else {
    ...
    public function updatetbyip(Request $request)
    {
        if ($request->isMethod('get')) {
            ...
            if ($validator->passes()) {
                ...
                if ($result) {
                    ...
                    ->update(['sapid' => $input['sapid'],'hostname' => $input['hostname'],'mac' => $input['mac']])
                    ->get();
                    if (($result_data)) {
